...
CGameControllerCTF::CGameControllerCTF(CGameContext *pGameServer) : IGameController(pGameServer)
{
	// game
	m_apFlags[0] = 0;
	m_apFlags[1] = 0;
...
void CGameControllerCTF::Snap(int SnappingClient)
{
	...
	if(m_apFlags[TEAM_RED])
	{
		if(m_apFlags[TEAM_RED]->IsAtStand())
			...
			pGameDataFlag->m_FlagCarrierRed = m_apFlags[TEAM_RED]->GetCarrier()->GetCID();
		else
		{
	...
	if(m_apFlags[TEAM_BLUE])
	{
		if(m_apFlags[TEAM_BLUE]->IsAtStand())
			...
			pGameDataFlag->m_FlagCarrierBlue = m_apFlags[TEAM_BLUE]->GetCarrier()->GetCID();
		else
		{
			...
			if(m_apFlags[fi ^ 1] && m_apFlags[fi ^ 1]->IsAtStand())
			{
				if(distance(F->GetPos(), m_apFlags[fi ^ 1]->GetPos()) < CFlag::ms_PhysSize + CCharacter::ms_PhysSize)
				{
					...
					str_format(aBuf, sizeof(aBuf), "flag_capture player='%d:%s' team=%d time=%.2f",
						F->GetCarrier()->GetCID(),
						Server()->ClientName(F->GetCarrier()->GetCID()),
