ZSTD = {
	...
	OptFind = function (name, required)
		local check = function(option, settings)
			...
			if ExecuteSilent("pkg-config --exists libzstd") == 0 then
				option.value = true
		...
		local apply = function(option, settings)
			if option.use_pkgconfig == true then
				settings.cc.flags:Add(RunCommand("pkg-config --cflags libzstd"))
				settings.link.flags:Add(RunCommand("pkg-config --libs libzstd"))
			end
		end
